...
%The emulation is programmed by first formally classifying processor instructions as privileged, 
%sensitive and innocuous \cite{10.1145/361011.361073}. An instruction is privileged if 
%it requires changing the mode of execution from user to supervisor mode. An instruction $i$ is 
%control-sensitive if, when applied to the current processor state $S_1$, results in a new state 
%$i(S_{1}) = S_{2}$ such that the execution mode of $S_{2}$ does not equal that of $S_{1}$ or if 
%$S_{2}$ has access to different resources than $S_1$ or both \cite{10.1145/361011.361073}. 
%An instruction is behaviour-sensitive if its execution depends on the execution mode or its 
%position in memory. An instruction is innocuous if it is not sensitive. 
%\citeauthor{10.1145/361011.361073} define a computer as virtualisable \enquote{[...] if
%the set of sensitive instructions for that computer is a subset of the set of privileged instructions} 
%\cite[6]{10.1145/361011.361073}. If this invariant holds, then the virtual machine monitor can trap 
%every sensitive instruction executed in a non-privileged virtual environment and emulate 
%its behaviour as if the instruction were executed on the underlying hardware. 

%Note how that is not the case made by \textcite{10.1145/368481.368502}. A kernel does not 
%act as a hardware emulator. \textcite{10.1145/361011.361073} formally define processor instructions as 
%privileged, sensitive and innocuous. They further prove that a virtual machine monitor can be 
%implemented on top of an instruction-set architecture if the set of sensitive 
%instructions is a subset of the set of privileged instructions. 


%Continuing the example above, the routine could serialise access to the 
%I/O device, safely extract the bit stream and propagate it back to the user process by modifying 
%a memory reference, thereby guaranteeing spatial noninterference between user processes.
%It is important to notice that the control program is allowed to access and modify space assigned to 
%user processes. Therefore, in this model, spatial noninterference is fully guaranteed between 
%user processes but unidirectionally guaranteed between user processes and the control program. 
%If the control program is compromised, then so are all user processes.

% The authors use an interruption system to halt the execution flow 
%of the processing unit and transfer control to a fixed location that represents the start address of 
%another routine.




%Noninterference across space 

%That is, the control program must wrap each process in an 
%isolated context that prevents errors local to that process from cascading to others. 
%To achieve this, the authors integrate an interruption system that can pause the instruction flow 
%of the processing unit. The interruption system uses indicator bits that represent different 
%program and machine conditions. For example, an indicator bit may denote that an I/O component has 
%emitted a signal or that an instruction failed to execute due to an invalid operand. 



%Furthermore, the authors introduced a mechanism 
%that prevented processes from monopolising the central processing unit by triggering an interrupt 
%once a counter-based clock expired. 
