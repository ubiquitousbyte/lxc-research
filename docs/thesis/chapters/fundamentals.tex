...
It is the primary unit of work in a computer system and rerpesents a program in execution.
...
contiguous memory region that represents the initialised data region. 
...
consumption and to allow two processes to communicate with each other. This has positive implications
on same-host density - a key performance metric for container runtimes. For example, the \verb|libc|
...
Instead, all processes can refer to the same physical frames. 

\subsection{Execution Scheduling}
\label{ch:fundamentals/processes/execution-scheduling}
A process is associated with a state object that dynamically changes as it executes.
On Linux, the state object is a simple unsigned integer that can hold 
any of the values listed in Table \ref{table:proc-states}.
