...
other tasks in the queue. In other words, $d$ equals the amount of time the task used 
the processing unit multiplied by its relative priority of task in the entire run queue.

The \verb|sched_entity| object of every process is also part of a 
a red-black tree whose search key is set to the virtual runtime property. Whenever the scheduler 
needs to schedule a new process for execution, it traverses the tree and finds the task with the 
lowest virtual runtime, which is always stored at the leftmost leaf node of the tree. 
The complexity of this operation equals the depth of the tree, i.e it takes $\Theta{(n\log{n})}$.
The scheduler uses a caching mechanism to store the leftmost node whenever a task is enqueued to the run queue, 
thereby reducing the traversal's cost to $\Theta{(1)}$.

%The Linux kernel has a priority value that measures a 
%process's willingness to be preempted. In other words, it represents how \enquote{nice} a process 
%is to all other processes on the system. The value ranges from $-20$ to $19$ with a default of $0$.
%Larger values correspond to lower priorities and smaller values correspond to higher priorities.
%The scheduler does not allocate a timeslice per process.
%Instead, it calculates how long a process should run as a function 
%\begin{equation}
%    f(n) = \sum_{i = 0}^{n} rq[i].priority
%\end{equation}
%of the number of currently runnable processes $n$, i.e the length of the running queue. 
%$f(n)$ sums the priority values of all tasks in the running queue $rq$ and denotes the queue's weight.
%The scheduler maintains the amount of time (in microseconds) allocated to a task and calls it 
%the task's virtual runtime. Whenever the system timer triggers an interrupt, the virtual runtime 
%of every task is updated.

%Instead, it calculates how long a process should run as a function of the number of currently 
%runnable processes, i.e the length of the running queue. The sum of all nice values for all processes
%in the running queue is used to compute the 

%The kernel uses the nice value as a weight to determine the amount of processor time a process 
%can consume. Whenever a task joins the running queue, the kernel checks if the currently running 
%process has consumed a larger proportion of the processor than the new process. If that is the case,
%it preempts the current process in favour of the new process. The scheduler is said to be 
%completely fair as it always attempts to give all processes a fair proportion of the processor.
