...
%%ost container runtimes utilise the noninterference
%boundary provided by \verb|fork()| because the function is part of the  
%POSIX standard and keeps process creation protable amongst compliant platforms. 
\begin{lstlisting}[style=syscalls, caption={Clone System Call}]
#define _GNU_SOURCE
#include <sched.h>
...
The system call wrapper expects the caller to provide a memory region to be used as the new process's stack. 
The wrapper pushes the function pointer onto the stack, validates the input parameters to the system 
call and transfers execution to the kernel by calling the system call. 
The kernel places the new process in the run queue and gives control back to the wrapper, which
checks if the process identifier returned by the kernel equals $0$. If it does, then the currently 
executing process is the child. The wrapper immediately pops the funtion off from the stack 
and calls it inside the new execution context \cite{aarch64/clone.S}. 
