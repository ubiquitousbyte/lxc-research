...
be in full control of the host system's memory. There are two primary methods to do this - \textit{shadow paging} and \textit{extended page tables}.
The former mechanism is considered first. 
...
The virtual machine monitor traps this attempt and transparently updates the page table pointer to point to 
...
addresses of the guest and updating the translation lookaside buffer.
...
The extended page table approach is faster because table walks are done in hardware - by the memory management unit. 
