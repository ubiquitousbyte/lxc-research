...
\begin{lstlisting}[style=syscalls, caption={Clone System Call Wrapper}]
#define _GNU_SOURCE
#include <sched.h>
...
The caller can also set the value of the thread-local storage register for the child process via the 
\verb|tls| parameter. However, the interpretation of this value is architecture-specific.
The primary limitation of the \verb|clone()| wrapper is the inability to call \verb|libc| functions 
from inside the child process. \verb|libc| expects the thread-local storage register to be configured 
in a specific way that cannot be reproduced. Consequently, the child may crash at arbitrary points 
